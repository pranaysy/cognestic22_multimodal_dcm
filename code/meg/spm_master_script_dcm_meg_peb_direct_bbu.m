...
addpath('/home/py01/Projects/dcmsurgery')
...
fits_dir = '/imaging/henson/Wakeman/pranay_does_things/codebase/dummy_bbu_psycustom';
...
DCM.B{1} = double(DCM.A{1} | DCM.A{2} | self_connections);

% C Matrix: Driving inputs
DCM.C = [1 1 0 0]';

% Save full model as a template
%dcm_full_file = fullfile(fits_dir, 'templates', 'DCMs', strcat(name, '.mat'));
...
for f=1:length(input_files)    
    ...
    GCM{f, 1}.name = sprintf('%s_sub-%02d_%s', name, f, modality); % Add subject identifier
end   
...
% Fit GCM
GCM = psy_spm_dcm_fit(GCM(:, 1), true);

% Switch back to base directory
cd(base_dir)

% Save fitted GCM
save(fullfile(fits_dir, sprintf('GCM_Full_%s', modality)), 'GCM')
...
% Save estimated PEB
save(fullfile(fits_dir, sprintf('PEB_Full_%s', modality)), 'PEB')
...
for f=1:length(input_files) 
    ...
    GCM2{f, 1} = DCM_Full; % Full model
    GCM2{f, 1}.xY.Dfile = input_files{f}; % Add path to subject f's data file
    GCM2{f, 1}.name = sprintf('%s_sub-%02d_%s', name, f, modality); % Add subject identifier
    
    % Specify modality
    GCM2{f, 1}.xY.modality = modality;

    % Propagate observation model priors
    GCM2{f, 1}.M.gE = GCM{f, 1}.Eg;
    GCM2{f, 1}.M.gC = GCM{f, 1}.Cg;

    % Propagate neuronal model priors
    GCM2{f, 1}.M.pE = GCM{f, 1}.Ep;
    GCM2{f, 1}.M.pC = GCM{f, 1}.Cp;

    % Propagate SNR hyperpriors (?)   
...
% Fit GCM
GCM2 = psy_spm_dcm_fit(GCM2(:, 1), true);

% Switch back to base directory
cd(base_dir)

% Save fitted GCM
save(fullfile(fits_dir, sprintf('GCM_Full_%s', modality)), 'GCM2')

%---------------------------------------------------------------------------------------
% STEP 3: Estimate 2nd-level PEB model
%---------------------------------------------------------------------------------------
M = []; M.Q = 'all'; % Random effects over all parameters
PEB2 = spm_dcm_peb(GCM2, M, {'B'});

% Save estimated PEB
save(fullfile(fits_dir, sprintf('PEB_Full_%s', modality)), 'PEB2')
...
for f=1:length(input_files) 
    ...
    GCM3{f, 1} = DCM_Full; % Full model
    GCM3{f, 1}.xY.Dfile = input_files{f}; % Add path to subject f's data file
    GCM3{f, 1}.name = sprintf('%s_sub-%02d_%s', name, f, modality); % Add subject identifier
    
    % Specify modality
    GCM3{f, 1}.xY.modality = modality;

    % Propagate observation model priors
    GCM3{f, 1}.M.gE = GCM2{f, 1}.Eg;
    GCM3{f, 1}.M.gC = GCM2{f, 1}.Cg;

    % Propagate neuronal model priors
    GCM3{f, 1}.M.pE = GCM2{f, 1}.Ep;
    GCM3{f, 1}.M.pC = GCM2{f, 1}.Cp;
...
% Fit GCM
GCM3 = psy_spm_dcm_fit(GCM3(:, 1), true);

% Switch back to base directory
cd(base_dir)

% Save fitted GCM
save(fullfile(fits_dir, sprintf('GCM_Full_%s', modality)), 'GCM3')

%---------------------------------------------------------------------------------------
% STEP 3: Estimate 2nd-level PEB model
%---------------------------------------------------------------------------------------
M = []; M.Q = 'all'; % Random effects over all parameters
PEB3 = spm_dcm_peb(GCM3, M, {'B'});

% Save estimated PEB
save(fullfile(fits_dir, sprintf('PEB_Full_%s', modality)), 'PEB3')
...
% Extract L
L = psy_spm_erp_L(DCM.Eg, DCM.M.dipfit);

% Invert leadfield
iL = pinv(L);

% Extract relevant channels from raw data (CAUTION: No tapering/centering!)
%y = D(DCM.M.dipfit.Ic, :, :);
y = DCM.xY.y;

% Project sensor data on to inverted Leadfield to get source activity
stc = {};
for con = 1:size(y,2) % For each trial
    stc{con, 1} = (iL * y{con}')';
end
outt{1} = stc;

% Get responses predicted by neuronal model
predicted = feval(DCM.M.IS, DCM.Ep, DCM.M, DCM.xU);
% 1, 7, 9 are populations per sources
%selector = sparse(1,9,1,1,9);
selector = full(DCM.Eg.J == 1);
pyrm_idx = find(kron(selector, ones([1, Nareas]))); 
for p=1:length(predicted)
    predicted{p} = full(predicted{p}(:, pyrm_idx)); % Columns for pyramidal output
end 
...
% Plot: STC
colors = {[45, 125, 210]./256, [1, 22, 39]./256};
figure
for k=1:4
...
%     hold on
%     
%     title(sprintf('%s (Inverted)', DCM.Sname{k}))
%     for c=1:2
%         
%         plot(DCM.xY.pst/1e3, squeeze(stc{c}(:, k)), 'Color', colors{c})
%         
%     end
%     hold off
    ...
    for c=1:2
        ...
        plot(DCM.xY.pst/1e3, squeeze(pred{c}(:, k)), 'Color', colors{c})
...
%     hold off
    ...
    pred = out{1};
    hold on
    title(sprintf('%s (Predicted)', DCM.Sname{k}))
    for c=1:2
